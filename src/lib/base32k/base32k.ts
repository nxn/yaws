...
const fromCharCodes = (function(fromCharCode, maxargs) {
...
export function encode(a: number[]): string {
...
export function encodeBytes(a: number[]|Uint8Array|string): string {
    var bits = a.length * 8;
    var at = typeof a == "string" ?
        function(i: number) { return a.charCodeAt(i) } :
        function(i: number) { return a[i] };
    var out = [];
    for (var p, q, r, i = 0; i < bits; i += 15) {
...
export function decodeBytes(s: string): string {
    var tailbits = s.charCodeAt(s.length - 1) - 0x2400;
    if (tailbits < 1 || tailbits > 15) {
      throw "Invalid encoding";
    }
