...
export interface IPuzzleController {
    ...
    openMostRecent(): void;
    generate(settings: IPuzzleSettings): void;
...
export class PuzzleController implements IPuzzleController {
    ...
    static create(board: IBoard, storage: IStorageCollection<Uint8Array>, provider: ISudokuProvider): IPuzzleController {
        return new PuzzleController(board, storage, provider);
    }
    ...
    save(info: IPuzzleInfo = this.board.getPuzzleInfo()) {
        if (info.storageId) {
    ...
    open(id: number) {
        ...
        this.board.setPuzzleInfo(this.storage.meta(id));
    }
    ...
    openMostRecent() {
        ...
        this.board.setPuzzleInfo(this.storage.meta(mostRecentId));
    }

    async generate(settings: IPuzzleSettings) {
        const response = await this.sudoku.generate(settings);
        this.loadValueArray(response.puzzle);

        const now = Date.now();
        this.board.setPuzzleInfo({
