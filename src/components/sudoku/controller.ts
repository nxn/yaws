...
const Controller: ICellController & ICursorController = {
    ...
    rowUp(board: IBoard) {
        board.cursor = offset(board.cursor.row.index).by(-1).in(board.rows).select(
            row => row.cells[board.cursor.column.index]
        );
    },
    
    columnLeft(board: IBoard) {
        board.cursor = offset(board.cursor.column.index).by(-1).in(board.columns).select(
    ...
    boxUp(board: IBoard) {
        board.cursor = offset(board.cursor.row.index).by(-gc.boxRowCount).in(board.rows).select(
            row => row.cells[board.cursor.column.index]
        );
    },
